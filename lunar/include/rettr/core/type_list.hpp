 ...
 */
#ifndef RETTR_TYPE_LIST_HPP
#define RETTR_TYPE_LIST_HPP
...
namespace rettr::helper::implements {
    template<typename...>
...
namespace rettr::helper::implements {
    ...
    struct type_list_concat_impl<type_list<Items1...>, type_list<Items2...>,
                type_list<Items3...>, Rest...> {
        using type = type_list_concat<type_list<Items1..., Items2..., Items3...>, Rest...>;
    };
}
...
namespace rettr::helper {
     ...
     */
    template<typename TypeList>
    RETTR_INLINE_CONSTEXPR std::size_t type_list_size_v = 0;
     ...
     */
    template<>
    RETTR_INLINE_CONSTEXPR std::size_t type_list_size_v<type_list<> > = 0;
     ...
     */
    template<typename... Types>
    RETTR_INLINE_CONSTEXPR std::size_t type_list_size_v<type_list<Types...> > = sizeof...(Types);
     ...
     */
    template<typename TypeList>
    struct type_list_size : std::integer_sequence<std::size_t, type_list_size_v<TypeList> > {
    };
    ...
    RETTR_INLINE_CONSTEXPR std::size_t count_type_v = static_cast<std::size_t>(-1);
    ...
    RETTR_INLINE_CONSTEXPR std::size_t count_type_v<Target, type_list<Args...> > =
            (0 + ... + (std::is_same_v<Target, Args> ? 1 : 0));
     ...
     */
    template<typename Target, typename TypeList>
    struct count_type : std::integral_constant<std::size_t, count_type_v<Target, TypeList> > {
    };
    ...
    struct type_find_unique<Find, type_list<First, Rest...> > {
        static constexpr std::size_t value =
                (count_type_v<Find, type_list<First, Rest...> > != 1)
                    ...
                    : (std::is_same_v<Find, First> ? 0 : 1 + type_find_unique<Find, type_list<Rest...> >::value);
    };
    ...
    struct is_type_in_list<Ty, List<First, Rest...> >
            : std::conditional_t<std::is_same_v<Ty, First>, std::true_type, is_type_in_list<Ty, List<Rest...> > > {
    };
     ...
     *        表示类型列表索引中“未找到”的常量。
     */
    RETTR_INLINE_CONSTEXPR std::size_t type_list_npos = static_cast<std::size_t>(-1);
}
...
namespace rettr::helper::implements {
    ...
    struct meta_quote_helper<std::void_t<Fx<Args...> >, Fx, Args...> {
        using type = Fx<Args...>;
    };
}
...
namespace rettr::helper::implements {
    ...
    struct type_list_apply_impl<Fn, std::integer_sequence<Ty, Idxs...> > {
        using type = type_list_invoke<Fn, std::integral_constant<Ty, Idxs>...>;
    };
    ...
    struct meta_as_integer_sequence_impl<type_list<std::integral_constant<Ty, Idxs>...> > {
...
namespace rettr::helper::implements {
    ...
    struct as_list_impl<List<Types...> > {
        using type = helper::type_list<Types...>;
    };

    template<typename Ty, Ty... Idxs>
    struct as_list_impl<std::integer_sequence<Ty, Idxs...> > {
        using type = type_list<std::integral_constant<Ty, Idxs>...>;
    };
}
...
namespace rettr::helper::implements {
    ...
    struct type_list_cartesian_product_impl<type_list<type_list<Items...>, Lists...> > {
        using type = type_list_join<type_list<type_list_transform<
...
namespace rettr::helper {
    ...
    private:
        ...
        struct prepend_if_unique<First, type_list<Us...> > {
            using type = std::conditional_t<(std::is_same_v<First, Us> || ...), type_list<Us...>, type_list<First, Us
                ...> >;
        };
...
namespace rettr::helper {
    ...
    struct select_type<Size, type_list<First, Rest...> > {
        using type = std::conditional_t<(Size <= sizeof(First)), First, typename select_type<Size, type_list<Rest
            ...> >::type>;
    };
