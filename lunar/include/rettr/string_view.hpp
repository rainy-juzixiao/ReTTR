...
namespace rettr {
    ...
    public:
        ...
        constexpr basic_string_view(const_pointer begin,
                                    const_pointer end) noexcept : data_(begin), size_(end - begin) {
        }

        template<typename R, std::enable_if_t<
            helper::has_data_v<helper::remove_cvref_t<R> > &&
            helper::has_size_v<helper::remove_cvref_t<R> >,
            int> = 0>
        ...
        template<
            ...
            std::enable_if_t<
                helper::is_contiguous_iterator_v<It> && helper::is_iterator_v<End> &&
                std::is_same_v<helper::iter_value_t<It>, value_type> &&
                !std::is_convertible_v<It, size_type> &&
                !std::is_convertible_v<End, size_type>,
                int> = 0>
        constexpr basic_string_view(It begin, End end) noexcept : data_{begin},
                                                                  size_{
        ...
        constexpr int compare(basic_string_view right) const noexcept {
            const size_type rlen = (std::min)(size_, right.size_);
        ...
        friend constexpr bool operator<(basic_string_view left, basic_string_view right) noexcept {
            auto lsize = left.size();
            auto rsize = right.size();
            auto result = Traits::compare(left.data(), right.data(), (std::min)(lsize, rsize));
            if (result < 0) {
                return true;
            }
            if (result > 0) {
        ...
        friend std::basic_string<CharType, Traits, Allocator>
        operator+(basic_string_view left, const std::basic_string<CharType, Traits, Allocator> &right) {
        ...
        template<typename Allocator>
        friend std::basic_string<CharType, Traits, Allocator>
        operator+(const std::basic_string<CharType, Traits, Allocator> &left, basic_string_view right) {
            std::basic_string<CharType, Traits, Allocator> result;
            result.reserve(left.size() + right.size());
            result.append(left.data(), left.size());
            result.append(right.data(), right.size());
        ...
        template<typename Allocator>
        friend std::basic_string<CharType, Traits, Allocator>
        operator+(basic_string_view left, std::basic_string<CharType, Traits, Allocator> &&right) {
            right.insert(0, left.data(), left.size());
            return std::move(right);
        }

        template<typename Allocator>
        friend std::basic_string<CharType, Traits, Allocator>
        operator+(std::basic_string<CharType, Traits, Allocator> &&left, basic_string_view right) {
            left.append(right.data(), right.size());
            return std::move(left);
        }

        friend std::basic_string<CharType, Traits>
        operator+(basic_string_view left, const CharType *right) {
            std::basic_string<CharType, Traits> result;
            auto right_len = traits_type::length(right);
            result.reserve(left.size() + right_len);
            result.append(left.data(), left.size());
        ...
        friend std::basic_string<CharType, Traits>
        operator+(const CharType *left, basic_string_view right) {
            std::basic_string<CharType, Traits> result;
            auto left_len = traits_type::length(left);
            result.reserve(left_len + right.size());
        ...
        friend std::basic_string<CharType, Traits>
        operator+(basic_string_view left, CharType right) {
            std::basic_string<CharType, Traits> result;
        ...
        friend std::basic_string<CharType, Traits>
        operator+(CharType left, basic_string_view right) {
            std::basic_string<CharType, Traits> result;
        ...
        friend std::basic_string<CharType, Traits>
        operator+(basic_string_view left, basic_string_view right) {
            std::basic_string<CharType, Traits> result;
            result.reserve(left.size() + right.size());
            result.append(left.data(), left.size());
            result.append(right.data(), right.size());
    ...
    basic_string_view(It, End) -> basic_string_view<helper::iter_value_t<It> >;
