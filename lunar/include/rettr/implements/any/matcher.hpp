...
namespace rettr::implements {
    ...
    public:
        ...
        explicit any_matcher(Handlers &&... hs) : handlers(std::forward<Handlers>(hs)...) {
        }
    ...
    private:
        ...
        bool try_to_invoke(basic_any const &this_, basic_any &result) const noexcept {
            ...
            using argument_t = typename type_at<0, typename function_traits<handler_t>::argument_list>::type;
            if constexpr (std::is_same_v<argument_t, any_default_match>) {
                return false; // 最后处理
            } else {
                auto &handler = std::get<I>(handlers);
                if (this_.template is<argument_t>()) {
                    if constexpr (std::is_void_v<function_return_type<handler_t> >) {
                        std::invoke(handler, this_.template as<argument_t>());
                    } else {
                        result = std::invoke(handler, this_.template as<argument_t>());
                    }
                    return true;
                }
                if (this_.template is_convertible<argument_t>()) {
                    if constexpr (std::is_void_v<function_return_type<handler_t> >) {
                        std::invoke(handler, this_.template convert<argument_t>());
                    } else {
                        result = std::invoke(handler, this_.template convert<argument_t>());
                    }
                    return true;
                }
                return false;
        ...
        static constexpr std::size_t eval_for_default_match_index(std::index_sequence<Idx...>) {
            using namespace helper;
            using type_list = type_list<typename type_at<0, function_argument_list<Handlers> >::type...>;
            std::size_t index{};
            ((std::is_same_v<remove_cvref_t<typename type_at<Idx, type_list>::type>, any_default_match>
                  ? index = Idx,
                    void()
                  : void()),
                ...);
        ...
        basic_any invoke_impl(basic_any const &a, std::index_sequence<I...>) const noexcept {
            ...
            constexpr std::size_t default_match_count = eval_for_default_match(
            ...
            if (!matched) {
                if constexpr (default_match_count == 1) {
                    constexpr std::size_t index = eval_for_default_match_index(
                        std::make_index_sequence<sizeof...(Handlers)>{});
