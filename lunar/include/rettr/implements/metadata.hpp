...
namespace rettr {
    ...
    public:
        ...
        metadata(metadata &&right) noexcept : key_{std::move(right.key_)}, value_{std::move(right.value_)} {
        }
        ...
        metadata(Ty1 &&key, Ty2 &&value) : value_(std::forward<Ty2>(value)) {
...
namespace rettr::implements {
    ...
    RETTR_INLINE_CONSTEXPR std::size_t metadata_count =
        helper::count_type_v<metadata, helper::type_list<Args...>>;

    RETTR_INLINE const metadata &find_metadata(const std::vector<metadata> &view, const any &key) {
        static const metadata empty;
        const auto it = std::find_if(view.begin(), view.end(), [&key](const metadata &meta) { return meta.key() == key; });
